...
const contactInfo = [
  {
    ...
    title: "Email",
    value: "bhavesh.baraiya.codes@gmail.com",
  ...
  useGSAP(() => {    
    ...
    gsap.fromTo(infoCards,
      { opacity: 0, x: -50, filter: "blur(5px)" },
      {
        ...
        scrollTrigger: { trigger: '.contact-content-wrapper', start: "top 60%", toggleActions: "play none none reverse" }
      }
    );
    ...
    const magneticItems = gsap.utils.toArray('.magnetic-wrap');
    ...
    >
      ...
      <div className="container mx-auto relative z-10 max-w-7xl">
        ...
        <div className="text-center mb-24 pt-5">
          <div className="contact-header-anim will-change-transform">
          ...
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-6">
            {contactInfo.map((info, index) => (
              <a 
                ...
                rel="noreferrer"
                className="contact-info-card magnetic-wrap will-change-transform relative group p-6 md:p-8 rounded-3xl bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 transition-all duration-500 overflow-hidden flex items-center gap-6 hover:border-brand-primary/30"
              >
                <div className="absolute inset-0 z-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  <div
                    className="absolute w-[300px] h-[300px] bg-brand-primary/10 dark:bg-brand-primary/15 rounded-full blur-[50px] -translate-x-1/2 -translate-y-1/2 will-change-transform"
                    style={{ left: "var(--mouse-x, 50%)", top: "var(--mouse-y, 50%)" }}
                  ></div>
                </div>

                <div className="relative z-10 flex-shrink-0 w-14 h-14 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-600 dark:text-zinc-400 group-hover:bg-brand-primary group-hover:text-white transition-colors duration-500">
                  {info.icon}
                </div>

                <div className="relative z-10">
                  <p className="text-sm font-mono text-zinc-500 dark:text-zinc-500 uppercase tracking-wider mb-1">
                    {info.title}
                  </p>
                  <h4 className="text-lg font-bold text-zinc-900 dark:text-white group-hover:text-brand-primary transition-colors duration-300">
                    {info.value}
                  </h4>
                </div>
              </a>
          ...
          <div className="lg:col-span-7">
            ...
            >
              <div className="absolute inset-0 z-0 pointer-events-none opacity-0 hover:opacity-100 transition-opacity duration-500">
                <div
                  className="absolute w-[600px] h-[600px] bg-brand-primary/5 dark:bg-brand-primary/10 rounded-full blur-[80px] -translate-x-1/2 -translate-y-1/2 will-change-transform"
                  style={{ left: "var(--mouse-x, 50%)", top: "var(--mouse-y, 50%)" }}
                ></div>
              </div>
             ...
             <form 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <label className="text-sm font-mono text-zinc-500 uppercase ml-2">Name</label>
                    <input 
                        type="text"
                        name="name"
                    ...
                    <div className="flex flex-col gap-2">
                    <label className="text-sm font-mono text-zinc-500 uppercase ml-2">Email</label>
                    <input 
                        type="email"
                        name="email"
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-mono text-zinc-500 uppercase ml-2">Subject</label>
                    <input 
                    type="text"
                    name="subject"
                ...
                <div className="flex flex-col gap-2">
                    <label className="text-sm font-mono text-zinc-500 uppercase ml-2">Message</label>
                    <textarea 
                    name="message"
                    rows="5"
