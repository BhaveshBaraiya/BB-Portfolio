...
const Hero = () => {
  ...
  return (
    ...
    >
      <div className="absolute top-1/4 left-1/4 w-64 h-64 md:w-96 md:h-96 bg-cyan-500/20 dark:bg-cyan-500/10 rounded-full blur-[80px] md:blur-[100px] pointer-events-none transition-colors duration-500" />
      <div className="absolute bottom-1/4 right-1/4 w-64 h-64 md:w-96 md:h-96 bg-indigo-500/20 dark:bg-indigo-500/10 rounded-full blur-[80px] md:blur-[100px] pointer-events-none transition-colors duration-500" />
      ...
      <div className={`container mx-auto px-6 relative z-10 flex flex-col ${isSecondSlide ? 'md:flex-row-reverse' : 'md:flex-row'} items-center justify-between gap-12`}>
        ...
        <div className="flex-1 max-w-2xl text-left">
          <div className="hero-content-anim min-h-[30px]">
            <TypeAnimation
              key={`sub-${currentSlide}`} 
              sequence={[slideData[currentSlide].sub, 1000, handleTypingComplete]}
              wrapper="p"
              cursor={true}              
          ...
          <h1 className="hero-content-anim text-4xl sm:text-5xl md:text-7xl font-extrabold tracking-tight text-zinc-900 dark:text-white mb-6 transition-colors duration-500 leading-[1.1]">
            {slideData[currentSlide].head}
          </h1>
          
          <p className="hero-content-anim text-base md:text-xl text-zinc-600 dark:text-zinc-400 leading-relaxed max-w-xl border-l-4 border-indigo-500 dark:border-indigo-400 pl-4 transition-colors duration-500">
        ...
        <div className={`hero-image flex-1 w-full flex  ${isSecondSlide ? 'justify-center md:justify-start' : 'justify-center md:justify-end'}`}>
          <div className="relative w-full max-w-[320px] aspect-square md:max-w-[550px] md:h-[600px] group">            
            ...
            <div className="relative w-full h-full overflow-hidden rounded-2xl transition-colors duration-500">
              <img 
